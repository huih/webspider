...
func formatDamsContent(removeDom []string, bodyContent *goquery.Selection) *goquery.Selection{
...
func getDamsItemData(objectStr string, query *goquery.Document, ctx *Context) {
			...
			"生成请求": {
				AidFunc: func(ctx *Context, aid map[string]interface{}) interface{} {
					for loop := aid["loop"].([2]int); loop[0] < loop[1]; loop[0]++ {
						ctx.AddQueue(&context.Request{
							Url:   "http://www.dams.org.cn/news-4-1.html",
				...
				ParseFunc: func(ctx *Context) {
					query := ctx.GetDom()
					getDamsItemData("li.span3", query, ctx)

...
var oneapmDeleteContent = []string {
...
func formatoneapmContent(removeDom []string, bodyContent *goquery.Selection) *goquery.Selection{
...
var oneapm = &Spider{
	Name:        "oneapm",
	Description: "oneapm博文采集 [http://www.oneapm.com/]",
	Host: "http://www.oneapm.com/",
		...
		Trunk: map[string]*Rule{
			...
			"详细页面": {
				...
				ParseFunc: func(ctx *Context) {
					...
					bodyContent = formatoneapmContent(ctx.Spider.GetRemoveDom(), bodyContent)
					...
					//remove bad content
					for _, item := range oneapmDeleteContent {
